# Hi there, I'm Khalid 👋

🔐 **Cryptography Enthusiast** | **Tech Learner**

Welcome to my GitHub profile! I’m passionate about designing secure encryption systems, analyzing cryptographic protocols, and developing privacy-preserving solutions. I love exploring cryptography algorithms and techniques, and I’m constantly learning to stay ahead in this fast-evolving field.

---

# 💫 About Me:
🛠️ I’m currently working on **cryptography projects** and **backend development with Django**.  
👥 I’m looking to collaborate on **encryption-focused open-source projects**.  
🤝 I’m looking for help with **advanced cryptanalysis and secure protocol design**.  
🌱 I’m currently learning **post-quantum cryptography, zero-knowledge proofs, and secure coding practices**.  
💬 Ask me about **symmetric/asymmetric encryption, hashing, and secure communications**.  
⚡ Fun fact: I won the **FCIT Hackathon 2024**!  

---

## 🌐 Socials:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/https://www.linkedin.com/in/khalidalabsi/) 
[![Email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:khaled.s.alabsi@gmail.com) 

---

# 💻 Tech Stack:
![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white) 
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) 
![Bash](https://img.shields.io/badge/bash-%23121011.svg?style=for-the-badge&logo=gnu-bash&logoColor=white) 
![PowerShell](https://img.shields.io/badge/PowerShell-%235391FE.svg?style=for-the-badge&logo=powershell&logoColor=white) 
![Django](https://img.shields.io/badge/django-%23092E20.svg?style=for-the-badge&logo=django&logoColor=white) 
![MySQL](https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white) 
![SQLite](https://img.shields.io/badge/sqlite-%2307405e.svg?style=for-the-badge&logo=sqlite&logoColor=white) 
![Figma](https://img.shields.io/badge/figma-%23F24E1E.svg?style=for-the-badge&logo=figma&logoColor=white) 
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![OpenSSL](https://img.shields.io/badge/OpenSSL-%23000000.svg?style=for-the-badge&logo=openssl&logoColor=white)

---

# 📊 GitHub Stats:
![](https://github-readme-stats.vercel.app/api?username=b0yx&theme=dark&hide_border=false&include_all_commits=false&count_private=false)<br/>
![](https://github-readme-streak-stats.herokuapp.com/?user=b0yx&theme=dark&hide_border=false)<br/>
![](https://github-readme-stats.vercel.app/api/top-langs/?username=b0yx&theme=dark&hide_border=false&include_all_commits=false&count_private=false&layout=compact)

---

## 💼 Current Projects:

- **Cryptography Toolkit**: Python scripts for AES, RSA, and ECC encryption/decryption.  
- **Secure Messaging Prototype**: Implemented end-to-end encrypted chat using Python.  
- **Hashing & Integrity Checker**: Tool for verifying file integrity using SHA-256 and HMAC.  

---

## 🛠️ Skills:

- **Cryptography Tools**: OpenSSL, PyCryptodome, Cryptography library, GPG.  
- **Programming & Scripting**: Python, C++, Bash, PowerShell.  
- **Protocol Analysis**: TLS/SSL, HTTPS, Signal Protocol.  
- **Operating Systems**: Linux, Windows.  

---

[![](https://visitcount.itsvg.in/api?id=b0yx&icon=0&color=0)](https://visitcount.itsvg.in)

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->
